...
export function NovelUpload({ onUpload }: { onUpload: () => void }) {
  ...
  // 计算剩余时间
  const calculateEstimatedTime = useCallback((speed: number, remaining: number) => {
    if (speed === 0) return 0
  ...
  // 完成上传
  const completeUpload = async (metadata: NovelUploadFormData): Promise<boolean> => {
    try {
    ...
    <Card className="w-full max-w-4xl mx-auto border-none p-0 gap-3">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          {/* <FileText className="h-4 w-4 text-orange-500 dark:text-orange-500" /> */}
          <span className='text-sm'>{t('上传小说')} <span className="text-red-500">*</span></span>
        </CardTitle>
        {/* <CardDescription>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {(uploadProgress.status === 'paused' || uploadProgress.status === 'error') && (
                  <Button
                    size="sm"
                    variant="outline"
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span>{completedChunks.size} / {chunks.length}</span>
              </div>
              <div>
                <span className="text-muted-foreground">预计剩余：</span>
                <span>{estimatedTime > 0 ? `${Math.ceil(estimatedTime)}s` : '计算中...'}</span>
              </div>
            </div>
            ...
            {uploadProgress.error && (
              <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-950 rounded-lg">
                ...
                <span className="text-sm text-red-600">{uploadProgress.error}</span>
              </div>
            )}
          </div>
        )}

        {/* <Separator className="divider-primary" /> */}
        ...
        {/* <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>小说标题 <span className="text-red-500">*</span></FormLabel>
                  <FormControl>
                    <Input
                      placeholder="请输入小说标题"
                      {...field}
        ...
        <div className='flex justify-center w-full mt-8'>
            <Button variant="secondary" onClick={handleUpload} className='text-orange-500 tracking-wide'>上传解析</Button>
        </div>
      </CardContent>
    </Card>
  )
