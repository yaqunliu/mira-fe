    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="text-center space-y-4">
          ...
          <h3 className="text-xl font-bold text-gray-700">{t("noShots")}</h3>
          <p className="text-gray-500 text-sm">
            {t("completeScenesFirst")}
          </p>
        </div>
      </div>
    );
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-4">
          <div className="flex-1 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
            <div className="text-xs text-blue-600 mb-1">{t("totalShots")}</div>
          ...
          <div className="flex-1 bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-4 border border-purple-200">
            <div className="text-xs text-purple-600 mb-1">{t("dubbed")}</div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <div className="relative aspect-video bg-gray-100 rounded overflow-hidden">
          {hasImage ? (
            <img
              src={shot.image_url}
              alt={t("startFrameLabel", { n: shotNumber })}
              className="w-full h-full object-cover"
            />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-gray-400 text-xs">{t("startFrame")}</span>
            </div>
          )}
          {/* 分镜编号角标 */}
        ...
        <div className="relative aspect-video bg-gray-100 rounded overflow-hidden">
          {endFrameImageUrl ? (
            <img
              ...
              alt={t('lastFrameOf', { n: shotNumber })}
              className="w-full h-full object-cover"
            />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-gray-400 text-xs">{t('lastFrame')}</span>
            </div>
          )}
        </div>
      </div>
