...
interface ShotImageHistoryDialogProps {
  isOpen: boolean;
  onClose: () => void;
  shotUuid: string;
  onSuccess: () => void;
...
export function ShotImageHistoryDialog({
  ...
  const { data: imageHistoryData, isLoading: isLoadingHistory } = useQuery({
    queryKey: ["shot-image-history", shotUuid],
    queryFn: async () => {
      const response = await shotApi.getImageHistory(shotUuid);
  ...
  const applyImageVersionMutation = useMutation({
    mutationFn: async (params: {
      ...
      return await shotApi.applyImageVersion(
        params.shotUuid,
        params.versionId,
        params.imageUrl,
  ...
  const handleApplyVersion = (item: ImageHistoryItem) => {
    ...
    applyImageVersionMutation.mutate({
      ...
      endFrameImageUrl: item.end_frame_image_url,
                            ...
                            <div className="flex gap-1.5 pt-1">
                              ...
                              {item.end_frame_image_url && (
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handlePreview(item.end_frame_image_url!)}
                                  className="h-6 text-[10px] px-2 border-gray-200 hover:bg-gray-50"
                                >
                                  <Eye className="h-3 w-3 mr-1" />
                                  预览尾帧
