...
interface SceneImageHistoryDialogProps {
  isOpen: boolean;
  onClose: () => void;
  sceneUuid: string;
  onSuccess: () => void;
...
export function SceneImageHistoryDialog({
  ...
  const { data: imageHistoryData, isLoading: isLoadingHistory } = useQuery({
    queryKey: ["scene-image-history", sceneUuid],
    queryFn: async () => {
      const response = await sceneApi.getImageHistory(sceneUuid);
  ...
  const applyImageVersionMutation = useMutation({
    mutationFn: async (params: {
      ...
      return await sceneApi.applyImageVersion(
        params.sceneUuid,
        params.versionId,
        params.imageUrl,
        params.imagePrompt
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-green-400 to-green-500">
