...
export function GoogleSignIn({ locale = 'zh' }: GoogleSignInProps) {
  ...
  return (
    <Button
      ...
      disabled={loading}
      className="w-full h-11 bg-white/50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 hover:bg-white dark:hover:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-200 hover:shadow-md"
    >
      {loading ? (
        <span className="text-gray-600 dark:text-gray-400">登录中...</span>
      ) : (
        <>
          ...
          <span className="text-gray-700 dark:text-gray-300 font-medium">使用 Google 登录</span>
        </>
      )}
    </Button>
  )
